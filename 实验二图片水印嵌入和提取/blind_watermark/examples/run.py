...
# 平移攻击
def translate_attack(input_path, output_path, x_shift=20, y_shift=20):
    img = cv2.imread(input_path)
    M = np.float32([[1, 0, x_shift], [0, 1, y_shift]])
    shifted = cv2.warpAffine(img, M, (img.shape[1], img.shape[0]))
    cv2.imwrite(output_path, shifted)
    print(f'平移攻击保存: {output_path}')

# 对比度攻击
def contrast_attack(input_path, output_path, ratio=1.5):
    img = Image.open(input_path).convert("RGB")
    enhancer = ImageEnhance.Contrast(img)
    enhanced_img = enhancer.enhance(ratio)
    enhanced_img.save(output_path)
    print(f'对比度攻击保存: {output_path}')
...
# %% 平移攻击
translate_attack('output/embedded.png', 'output/parr.png', x_shift=30, y_shift=30)

# 直接提取
bwm1 = WaterMark(password_wm=1, password_img=1)
try:
    wm_extract = bwm1.extract('output/parr.png', wm_shape=len_wm, mode='str')
    print(f"平移攻击提取结果：{wm_extract}")
    assert wm == wm_extract, '提取水印和原水印不一致'
except Exception as e:
    print(f"平移攻击下提取失败: {e}")

# %% 对比度攻击
contrast_attack('output/embedded.png', 'output/contrast.png', ratio=1.8)

# 提取水印
bwm1 = WaterMark(password_wm=1, password_img=1)
try:
    wm_extract = bwm1.extract('output/contrast.png', wm_shape=len_wm, mode='str')
    print(f"对比度攻击提取结果：{wm_extract}")
    assert wm == wm_extract, '提取水印和原水印不一致'
except Exception as e:
    print(f"对比度攻击下提取失败: {e}")
