// sm4_optimized.cpp - 包含T-table、AVX2并行、AVX512-GFNI+VPROLD优化结构

#include <immintrin.h>
#include <cstdint>
#include <cstring>
#include <iostream>
#include <iomanip>

// ----------- 常量定义（FK, CK, Sbox） -------------

static const uint32_t FK[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};

static const uint32_t CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

alignas(16) static const uint8_t Sbox[256] = {
    
};

static uint32_t T[256];

inline uint32_t RotL(uint32_t x, int n) {
    return (x << n) | (x >> (32 - n));
}

inline uint32_t L(uint32_t b) {
    return b ^ RotL(b, 2) ^ RotL(b, 10) ^ RotL(b, 18) ^ RotL(b, 24);
}

inline uint32_t tau(uint32_t A) {
    return (Sbox[(A >> 24) & 0xFF] << 24) | (Sbox[(A >> 16) & 0xFF] << 16) |
        (Sbox[(A >> 8) & 0xFF] << 8) | Sbox[A & 0xFF];
}

// ----------- T-table 预处理 -------------

void build_t_table() {
    for (int i = 0; i < 256; ++i) {
        uint32_t x = i;
        x = Sbox[x];
        x = (x << 24) | (x << 16) | (x << 8) | x;
        T[i] = L(x);
    }
}

uint32_t F(uint32_t x0, uint32_t x1, uint32_t x2, uint32_t x3, uint32_t rk) {
    uint32_t tmp = x1 ^ x2 ^ x3 ^ rk;
    return x0 ^ T[(tmp >> 24) & 0xFF] ^ T[(tmp >> 16) & 0xFF] ^
        T[(tmp >> 8) & 0xFF] ^ T[tmp & 0xFF];
}

// ----------- 密钥扩展 -------------

void sm4_key_schedule(const uint8_t key[16], uint32_t rk[32]) {
    uint32_t K[36];
    for (int i = 0; i < 4; ++i) {
        K[i] = ((uint32_t)key[4 * i] << 24) | ((uint32_t)key[4 * i + 1] << 16) |
            ((uint32_t)key[4 * i + 2] << 8) | ((uint32_t)key[4 * i + 3]);
        K[i] ^= FK[i];
    }

    for (int i = 0; i < 32; ++i) {
        uint32_t tmp = K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i];
        uint32_t b = tau(tmp);
        uint32_t Lkey = b ^ RotL(b, 13) ^ RotL(b, 23);
        K[i + 4] = K[i] ^ Lkey;
        rk[i] = K[i + 4];
    }
}

// ----------- 单块加密（T-table版本） -------------

void sm4_encrypt_block(const uint8_t in[16], uint8_t out[16], const uint32_t rk[32]) {
    uint32_t X[36];
    for (int i = 0; i < 4; ++i) {
        X[i] = ((uint32_t)in[4 * i] << 24) | ((uint32_t)in[4 * i + 1] << 16) |
            ((uint32_t)in[4 * i + 2] << 8) | ((uint32_t)in[4 * i + 3]);
    }

    for (int i = 0; i < 32; ++i) {
        X[i + 4] = F(X[i], X[i + 1], X[i + 2], X[i + 3], rk[i]);
    }

    for (int i = 0; i < 4; ++i) {
        uint32_t v = X[35 - i];
        out[4 * i] = (v >> 24) & 0xFF;
        out[4 * i + 1] = (v >> 16) & 0xFF;
        out[4 * i + 2] = (v >> 8) & 0xFF;
        out[4 * i + 3] = v & 0xFF;
    }
}

// ----------- AVX512 GFNI/VPROLD 轮函数 -------------

__m512i sm4_gfni_sbox(__m512i input) {
    __m512i aff_mtx = _mm512_set1_epi64(0x1F1F1F1F1F1F1F1F);
    return _mm512_gf2p8affine_epi64_epi8(input, aff_mtx, 0x00);
}

__m512i rotate_left_32(__m512i x, int n) {
    return _mm512_rol_epi32(x, n);
}


int main() {
    uint8_t key[16] = {
        0x01, 0x23, 0x45, 0x67,
        0x89, 0xab, 0xcd, 0xef,
        0xfe, 0xdc, 0xba, 0x98,
        0x76, 0x54, 0x32, 0x10
    };

    uint8_t input[16] = {
        0x01, 0x23, 0x45, 0x67,
        0x89, 0xab, 0xcd, 0xef,
        0xfe, 0xdc, 0xba, 0x98,
        0x76, 0x54, 0x32, 0x10
    };

    uint32_t rk[32];
    uint8_t output[16];

    build_t_table();
    sm4_key_schedule(key, rk);
    sm4_encrypt_block(input, output, rk);

    std::cout << "Ciphertext: ";
    for (int i = 0; i < 16; ++i) {
        std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)output[i];
    }
    std::cout << std::endl;

    return 0;
}
